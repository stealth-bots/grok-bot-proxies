import { createHmac } from 'node:crypto';
import { afterEach, describe, expect, it, vi } from 'vitest';
import vercelHandler from '../api/index';
import { handleRequest, verifySlackSignature } from '../src/index';

const WEBHOOK_URL = 'https://api2.cursor.sh/automations/webhook/00000000-0000-0000-0000-000000000000';
const WEBHOOK_KEY = 'crsr_test_key_not_real';

const baseEnv = {
	CURSOR_WEBHOOK_URL: WEBHOOK_URL,
	CURSOR_WEBHOOK_KEY: WEBHOOK_KEY,
};

type CapturedFetch = {
	url: string;
	method: string;
	authorization: string | null;
	contentType: string | null;
	body: string;
};

afterEach(() => {
	vi.unstubAllGlobals();
	vi.restoreAllMocks();
});

describe('GET health', () => {
	it('returns a short health string and no secrets', async () => {
		const response = await handleRequest(new Request('https://proxy.example/'), baseEnv);
		expect(response.status).toBe(200);
		const text = await response.text();
		expect(text).toBe('ok');
		expect(text).not.toContain(WEBHOOK_KEY);
		expect(text).not.toContain('crsr_');
	});
});

describe('url_verification', () => {
	it('echoes the challenge as text/plain and does not forward to Cursor', async () => {
		const fetchMock = mockCursorFetch();
		const challenge = '3eZbrw1aBm2rZgRNFdxV2595E9CY3gmdALWMmHkvFXO7tYXAYM8P';
		const response = await handleRequest(
			new Request('https://proxy.example/', {
				method: 'POST',
				headers: { 'content-type': 'application/json' },
				body: JSON.stringify({
					type: 'url_verification',
					challenge,
					token: 'unused-legacy-token',
				}),
			}),
			baseEnv,
		);

		expect(response.status).toBe(200);
		expect(response.headers.get('content-type')).toMatch(/text\/plain/);
		expect(await response.text()).toBe(challenge);
		expect(fetchMock).not.toHaveBeenCalled();
	});
});

describe('event forward', () => {
	it('POSTs the raw body to CURSOR_WEBHOOK_URL with a Bearer header and never puts the key in the URL', async () => {
		const captured: CapturedFetch[] = [];
		mockCursorFetch(captured, { status: 200, body: JSON.stringify({ success: true, runUuid: 'test-run' }) });

		const slackBody = JSON.stringify({
			type: 'event_callback',
			event: { type: 'app_mention', text: 'hello' },
		});

		const response = await handleRequest(
			new Request('https://proxy.example/', {
				method: 'POST',
				headers: { 'content-type': 'application/json' },
				body: slackBody,
			}),
			baseEnv,
		);

		expect(response.status).toBe(200);
		expect(captured).toHaveLength(1);
		expect(captured[0].method).toBe('POST');
		expect(captured[0].url).toBe(WEBHOOK_URL);
		expect(captured[0].url).not.toContain(WEBHOOK_KEY);
		expect(captured[0].url).not.toMatch(/[?&](key|token|authorization)=/i);
		expect(new URL(captured[0].url).search).toBe('');
		expect(captured[0].authorization).toBe(`Bearer ${WEBHOOK_KEY}`);
		expect(captured[0].contentType).toMatch(/application\/json/);
		expect(captured[0].body).toBe(slackBody);
	});

	it('returns 502 when Cursor fails', async () => {
		mockCursorFetch([], { status: 500, body: 'nope' });

		const response = await handleRequest(
			new Request('https://proxy.example/', {
				method: 'POST',
				headers: { 'content-type': 'application/json' },
				body: JSON.stringify({ type: 'event_callback' }),
			}),
			baseEnv,
		);

		expect(response.status).toBe(502);
	});
});

describe('Slack request signing', () => {
	const signingSecret = 'test_signing_secret_not_real';
	const body = JSON.stringify({ type: 'event_callback', event: { type: 'message' } });

	it('forwards when SLACK_SIGNING_SECRET is unset', async () => {
		const captured: CapturedFetch[] = [];
		mockCursorFetch(captured);

		const response = await handleRequest(
			new Request('https://proxy.example/', {
				method: 'POST',
				headers: { 'content-type': 'application/json' },
				body,
			}),
			baseEnv,
		);

		expect(response.status).toBe(200);
		expect(captured).toHaveLength(1);
	});

	it('rejects a bad signature before forwarding when the signing secret is set', async () => {
		const fetchMock = mockCursorFetch();
		const timestamp = String(Math.floor(Date.now() / 1000));

		const response = await handleRequest(
			new Request('https://proxy.example/', {
				method: 'POST',
				headers: {
					'content-type': 'application/json',
					'x-slack-request-timestamp': timestamp,
					'x-slack-signature': 'v0=deadbeef',
				},
				body,
			}),
			{ ...baseEnv, SLACK_SIGNING_SECRET: signingSecret },
		);

		expect(response.status).toBe(401);
		expect(fetchMock).not.toHaveBeenCalled();
	});

	it('Vercel handler forwards with a Bearer header from process.env and never puts the key in the URL', async () => {
		vi.stubEnv('CURSOR_WEBHOOK_URL', WEBHOOK_URL);
		vi.stubEnv('CURSOR_WEBHOOK_KEY', WEBHOOK_KEY);
		vi.stubEnv('SLACK_SIGNING_SECRET', '');

		const captured: CapturedFetch[] = [];
		mockCursorFetch(captured);

		const slackBody = JSON.stringify({ type: 'event_callback', event: { type: 'message.im' } });
		const response = await vercelHandler.fetch(
			new Request('https://proxy.example/', {
				method: 'POST',
				headers: { 'content-type': 'application/json' },
				body: slackBody,
			}),
		);

		expect(response.status).toBe(200);
		expect(captured).toHaveLength(1);
		expect(captured[0].url).toBe(WEBHOOK_URL);
		expect(captured[0].url).not.toContain(WEBHOOK_KEY);
		expect(new URL(captured[0].url).search).toBe('');
		expect(captured[0].authorization).toBe(`Bearer ${WEBHOOK_KEY}`);
	});

	it('accepts a valid Slack signature computed independently', async () => {
		const timestamp = '1531420618';
		const expected = await verifySlackSignature({
			signingSecret,
			signature: nodeSlackSignature(signingSecret, timestamp, body),
			timestamp,
			rawBody: body,
			nowSeconds: Number(timestamp),
		});
		expect(expected).toBe(true);
	});
});

function mockCursorFetch(captured: CapturedFetch[] = [], reply: { status: number; body: string } = { status: 200, body: '{"success":true}' }) {
	const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
		const request = new Request(input, init);
		captured.push({
			url: request.url,
			method: request.method,
			authorization: request.headers.get('authorization'),
			contentType: request.headers.get('content-type'),
			body: await request.text(),
		});
		return new Response(reply.body, { status: reply.status });
	});
	vi.stubGlobal('fetch', fetchMock);
	return fetchMock;
}

function nodeSlackSignature(secret: string, timestamp: string, rawBody: string): string {
	const hex = createHmac('sha256', secret).update(`v0:${timestamp}:${rawBody}`).digest('hex');
	return `v0=${hex}`;
}
